...
import { iconSvgPaths } from '@/lib/icon-svgs'
    ...
    <>
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box}
        :root{--bg:#f3f3f1;--bg-2:#ebeae6;--ink:#0a0a0a;--ink-2:#1a1a1a;--muted:#6b6b66;--line:rgba(10,10,10,0.07);--line-2:rgba(10,10,10,0.12);--card:#ffffff;--card-ink:#0a0a0a}
        ...
        .link{display:flex;align-items:center;gap:14px;width:100%;padding:10px 18px 10px 10px;border-radius:999px;background:var(--card);color:var(--card-ink);text-decoration:none;border:1px solid rgba(10,10,10,.08);box-shadow:0 1px 0 rgba(255,255,255,.9) inset,0 8px 18px -8px rgba(0,0,0,.12);transition:transform .2s,box-shadow .25s,border-color .25s}
        .link:hover{transform:translateY(-1px);border-color:#A8D156;box-shadow:0 1px 0 #fff inset,0 0 0 1px #A8D156,0 0 10px rgba(168,209,86,.85),0 0 26px rgba(168,209,86,.6),0 0 50px rgba(168,209,86,.35)}
        .link:hover .arrow{transform:translateX(2px) rotate(-45deg)}
        @media(prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important}}
      `}</style>

      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute w-[580px] h-[580px] rounded-full opacity-30" style={{ background: '#A8D156', filter: 'blur(130px)', top: -180, left: -160 }} />
        <div className="absolute w-[580px] h-[580px] rounded-full opacity-25" style={{ background: '#A8D156', filter: 'blur(130px)', bottom: -220, right: -140 }} />
      ...
      <main className="relative z-10 max-w-[480px] mx-auto px-5 pt-12 pb-16 flex flex-col items-center">
        <header className="flex flex-col items-center text-center mb-8">
          ...
          <div className="reveal mb-1" style={{ animationDelay: '.05s' }}>
            <img src={template.logo_url} alt={template.name} style={{ width: 170, height: 'auto', display: 'block' }} />
          </div>
        ...
        <nav className="w-full flex flex-col gap-2.5" aria-label="Links">
          {ctas.map((cta, i) => (
            ...
            >
              <span className="w-10 h-10 rounded-full bg-[var(--bg-2)] border border-[var(--line-2)] flex items-center justify-center flex-shrink-0 text-[var(--ink-2)]">
                {ctaIcon(cta.icon)}
              </span>
              <span className="flex-1 flex flex-col gap-0.5">
                <span className="text-[15px] font-medium tracking-[-0.01em]">{cta.label}</span>
                {cta.meta && <span className="text-xs text-[var(--muted)]">{cta.meta}</span>}
              </span>
              <span className="arrow w-7 h-7 rounded-full bg-zinc-100 flex items-center justify-center flex-shrink-0 transition-transform duration-200">
        ...
        <footer className="mt-11 flex flex-col items-center gap-2 opacity-60">
          {/* eslint-disable-next-line @next/next/no-img-element */}
